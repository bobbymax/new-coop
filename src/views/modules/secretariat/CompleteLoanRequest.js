...
const CompleteLoanRequest = () => {
  ...
  const getSlicedMonths = (count) => {
    ...
    const newMonths = months.slice(state.month);
    const newArr = [];
  ...
  console.log(paymentPlan, installments);
  ...
  console.log(budget, request, state.month);
  ...
  return (
    <div className="row">
      ...
      <div className="col-md-4 mt-3">
        ...
        >
          <CustomSelectOptions
            label="SELECT REPAYMENT MONTH"
            value=""
            disabled
          />
          {getMonthsArr().map((month, i) => (
            <CustomSelectOptions key={i} value={i} label={month} />
          ))}
        </CustomSelect>
      </div>
...
export default CompleteLoanRequest;
